...
class SQLHelper extends PDO{
    ...
    public function __construct( $mode = MYSQL ){
        ...
        switch( $mode ){
        case 1: // SQLITE
            $this->connectionString = 'sqlite:'.PATH.'/database/db.sqlite'; // SQLITE
            try{
    ...
    private function createTables()
    {
        ...
        switch( $this->mode ){
        case 1:
            $this->readSQLFile($cmds, PATH."/database/script.sql");
            break;
        case 2:
            $this->readSQLFile($cmds, PATH."/database/tables.sql");
            $this->readSQLFile($cmds, PATH."/database/initdata.sql");
            break;
        }
        // leer archivo con zonas
        $this->readSQLFile($cmds, PATH."/database/zonas.sql");
